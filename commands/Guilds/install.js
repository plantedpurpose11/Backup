...
module.exports = class extends Command {
    ...
    async run(message, [ID]) {
        ...
        const managedRole = findManagedRole(myRoles);
        
        if (
            (managedRole && managedRole.rawPosition &&
            ((message.guild.roles.size - 1) - managedRole.rawPosition !== 0))
        ) 
            return message.channel.send(`❌ I do not have enough permissions to install a backup. Please make sure the role called \`${managedRole ? managedRole.name : 'managed'}\` is above all the others in \`Server Settings > Roles\` and try this command again.`);
        else if (
            (!managedRole || !managedRole.rawPosition) &&
            myRoles.size > 0 &&
            ((message.guild.roles.size - 1) - myRoles.highest.position !== 0)
        )
            return message.channel.send(`❌ I do not have enough permissions to install a backup. Please make sure my highest role is above all the others in \`Server Settings > Roles\` and try this command again.`);
